<template>
	<view  v-if="vuex_user.system == 1">
		<view class="u-p-l-20 u-p-r-20">
			<view class="tab u-m-t-20" v-for="(item, index) in cpsList" :key="index">
...
<script>
	export default {
		...
		async onShow() {
			var that = this
			if(uni.getStorageSync("update_cps_flag")){
				that.getConfigList()
				uni.removeStorageSync('update_cps_flag')
			}
		},
		methods: {
			async getConfigList(){
				...
				that.cpsList = list.result.data
				
				if(that.cpsList.length == 0) that.noData = true
			...
			addClick(){
				uni.navigateTo({
					url: '/pages/system/cps/cpsEdit?id=-1'
				})
			},
			async sheetChange(index){
				...
				if(that.sheetList[index].text === '编辑'){
					uni.navigateTo({
						url: '/pages/system/cps/cpsEdit?id='+that.cpsId
					})
