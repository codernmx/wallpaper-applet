...
exports.main = async (event, context) => {
	...
	let wx_user = await db.collection('wx_user').where({openid: res.openid}).get();
	let newdate = {};
	// 判断用户是否存在
	if (wx_user.data.length == 0) {
		...
		// 判断是否是被邀请用户
		if (event.userId) {
			let integrals = await db.collection('wx_config').where({'key': 'inviteIntegral'}).get()
		...
		let user_update = await db.collection('wx_user').where({openid: res.openid}).update({
			'name': event.name,
			'avatar': event.avatar
		})
	}
	...
	// 查询最新的数据
	let result = await db.collection('wx_user').where({openid: res.openid}).get();
